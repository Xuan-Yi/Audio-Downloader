...
path = StringVar(value="")  # destination path
...
format_combo = ttk.Combobox(format_frame, value=[
    "wav", 'flac', "m4a", "mp3"], state='readonly', width=6, font=utility_fontstyle)
    ...
    if not perfect:
        if not internet_connected():
            messagebox.showerror('No internet connection!',
                                 'Converting failed, please check your internet connection.')
        else:
            messagebox.showerror('Something went wrong.',
                                 'Unexpected error occurred, converting failed.')

    URL_entry.config(state='normal')
    URL_btn.config(state='active', text='+')
    normalize_entry.config(state='normal')
